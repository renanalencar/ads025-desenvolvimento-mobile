# Práticas — Aula 4 (domínio Pet)

> **Disciplina:** Desenvolvimento Mobile (2026.2.DM) — CESAR School
> **Domínio deste arquivo:** App de Gestão e Rotina Pet (`Pet`).
> **Está no outro projeto?** Se o seu app é o **Rastreador de Micro-hábitos e Condicionamento Físico**, use `exercises.md` — mesma estrutura, mesmos conceitos, domínio `Habito`.

| Projeto | Repositório | Branch da disciplina |
|---|---|---|
| App de Gestão e Rotina Pet | `pet-routine-expo` | `feature/pratica_04` |

## Como usar este arquivo

- **Parte 1 — Práticas guiadas** (em sala, ~38 min): foco em **acurácia**. São propositalmente simples. O objetivo é usar o conceito **corretamente**, não criativamente. Chame o professor quando travar.
- **Parte 2 — Práticas aplicadas** (para casa): exigem **decisão**, não repetição.
...
> 🧩 **Como usar os scaffolds:** a estrutura do arquivo, os imports e os nomes dos estilos já estão lá — o que falta é o miolo, que é onde está o conceito da aula. **Apague o comentário `// TODO` quando resolver aquele ponto.** Um arquivo sem nenhum `TODO` é uma prática concluída, e é assim que o professor confere rápido quem parou onde.
...
> ⚠️ **Regras de escrita válidas para todas as práticas** — as cinco primeiras vêm da Aula 3 e continuam valendo; as três últimas são desta aula:
...
**Setup das práticas 3 a 7** (projeto de sandbox, descartável — as práticas 1, 2 e 4 são no papel):
...
**Setup da Prática Aplicada 1** (seu projeto de verdade):

```bash
git clone <url-do-seu-repositorio> pet-routine-expo
cd pet-routine-expo
git checkout -b feature/pratica_04
npm install
npx expo start
```
...
# Parte 1 — Práticas guiadas

## Prática 1 — Associação: qual prop resolve o sintoma
...
| # | **Sintoma** | | **Prop** |
|---|---|---|---|
| 1 | "O ✕ de remover pet é minúsculo, eu erro o toque" | ( ) | a. `ListEmptyComponent` |
| 2 | "A tela fica em branco quando não tenho nenhum pet" | ( ) | b. `stickySectionHeadersEnabled` |
| 3 | "Registrei um passeio, mas a lista não mudou de aparência" | ( ) | c. `hitSlop` |
| 4 | "O item acende sozinho enquanto eu rolo a lista" | ( ) | d. `ItemSeparatorComponent` |
| 5 | "Sobra um espaço estranho depois do último pet" | ( ) | e. `extraData` |
| 6 | "O título do grupo gruda no iPhone e não no Android" | ( ) | f. `keyExtractor` |
| 7 | "O 'passeio concluído' pulou para o pet errado depois que filtrei" | ( ) | g. `unstable_pressDelay` |
...
const PETS: Pet[] = [ /* 200 itens */ ];

export default function TelaPets() {
  const [pets, setPets] = useState(PETS);

  function registrarPasseio(id: string) {
    const alvo = pets.find((p) => p.id === id);
    if (alvo) alvo.statusPasseio = 'concluido';
    setPets(pets);
  }
  ...
  return (
    <ScrollView>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            onPressIn={() => registrarPasseio(item.id)}
...
## Prática 3 — Reproduza: o botão tocável com feedback
...
<View style={{ flex: 1, justifyContent: 'center', padding: 24, gap: 16 }}>
  <BotaoAcao rotulo="Registrar passeio" onPressionar={() => setSalvo(true)} />
  <BotaoAcao rotulo="Registrar passeio" onPressionar={() => setSalvo(true)} desabilitado />
  <Text>{salvo ? 'Registrado!' : 'Ainda não registrado'}</Text>
</View>
```

**Confira:** ao **arrastar o dedo para fora** do botão antes de soltar, o texto **não** pode mudar para "Registrado!". Se mudar, o TODO 1 está errado.

---

## Prática 4 — Previsão: o ciclo de vida do toque
...
## Prática 5 — Complete: a `FlatList` inteira

**Nível:** ⭐⭐ · **Tempo:** 10 min

Monte uma tela de pets com `FlatList` completa: cabeçalho, separador, estado vazio, itens tocáveis e chave correta.
...
const MOCK: Pet[] = [
  { id: 'p1', nome: 'Aurora', especie: 'cachorro', statusPasseio: 'pendente',  idadeMeses: 18 },
  { id: 'p2', nome: 'Bidu',   especie: 'cachorro', statusPasseio: 'concluido', idadeMeses: 54 },
  { id: 'p3', nome: 'Nina',   especie: 'gato',     statusPasseio: 'pendente',  idadeMeses: 7 },
  { id: 'p4', nome: 'Kiwi',   especie: 'ave',      statusPasseio: 'cancelado', idadeMeses: 30 },
];
...
type ItemProps = { pet: Pet; onAlternar: (id: string) => void };

function ItemPet({ pet, onAlternar }: ItemProps) {
  const passeouHoje = pet.statusPasseio === 'concluido';
  return (
    <Pressable
    ...
    >
      <View style={styles.itemTexto}>
        <Text style={[styles.nome, passeouHoje && styles.nomeConcluido]}>{pet.nome}</Text>
        <Text style={styles.legenda}>{pet.especie} · {pet.idadeMeses} meses</Text>
      </View>
      <Text style={styles.marcador}>{passeouHoje ? '✓' : '○'}</Text>
    </Pressable>
  );
}

export default function TelaPets() {
  const [pets, setPets] = useState<Pet[]>(MOCK);
  ...
  return (
    <View style={styles.tela}>
      <FlatList
        data={pets}
        renderItem={({ item }) => <ItemPet pet={item} onAlternar={alternar} />}
      ...
      />
      <Text style={styles.limpar} onPress={() => setPets([])}>esvaziar a lista</Text>
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  nome:            { fontSize: 16, fontWeight: '600', color: '#191919' },
  nomeConcluido:   { textDecorationLine: 'line-through', color: '#6b6459' },
...
2. Tocar num item risca o nome? (Se não: você mutou o objeto.)
3. Tocar em "esvaziar a lista" mostra o estado vazio? (Se aparece só branco: TODO 2.)

---

## Prática 6 — Complete: `SectionList` a partir de dados planos

**Nível:** ⭐⭐ · **Tempo:** 7 min

Reaproveite o mesmo `MOCK` e o mesmo `ItemPet` da Prática 5. Agrupe por status do passeio.
...
type Secao = { title: string; data: Pet[] };

function agruparPorStatus(pets: Pet[]): Secao[] {
  const rotulos: Record<StatusPasseio, string> = {
    // TODO 1: um rótulo legível por status
  };
  const ordem: StatusPasseio[] = [/* TODO 2: a ordem em que os grupos aparecem na tela */];
...
export default function TelaSecoes({ pets }: { pets: Pet[] }) {
  return (
    <View style={styles.tela}>
      <SectionList
        sections={/* TODO 5 */}
        renderItem={/* TODO 6: reaproveite ItemPet */}
        renderSectionHeader={/* TODO 7: leia o campo que você inventou no TODO 3 */}
        renderSectionFooter={/* TODO 8: "N pet(s)" — dica: a seção conhece o próprio data */}
...
**Pergunta final (responda em comentário no arquivo):** você precisou mudar **alguma coisa** dentro do `ItemPet` para ele funcionar no `SectionList`? O que isso te diz sobre a diferença entre os dois componentes?

---

## Prática 7 — Corrija: a lista que não atualiza
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable onPress={() => alternarSelecao(item.id)} style={styles.item}>
            <Text>{selecionados.has(item.id) ? '☑' : '☐'} {item.nome}</Text>
          </Pressable>
        )}
      />
    </View>
...
# Parte 2 — Práticas aplicadas

## Prática Aplicada 1 — A tela de lista de verdade

**Nível:** ⭐⭐⭐ · **Tempo:** 3–4 h · **Entrega:** branch `feature/pratica_04` do repositório do grupo

### Contexto

Na Aula 3 vocês deixaram a tela do projeto **apresentável**: tokens, `CardPet`, Flexbox, um formulário. Mas ela só sabia mostrar os pets que vocês digitaram na mão dentro do JSX — e o botão era um remendo.
...
1. **Uma tela principal** que lista os pets do usuário com **`SectionList`**, agrupando por um critério que faça sentido para o produto (status do passeio, espécie ou faixa etária — escolham e justifiquem no README).
2. **Item tocável** com `Pressable`, reaproveitando o `CardPet` da Aula 3 por dentro. O toque alterna o status do passeio.
3. **Ação secundária no toque longo** (`onLongPress`) — por exemplo, remover o pet da lista.
4. **Os quatro estados da lista**, todos implementados:
   ...
   - **filtrado sem resultado** — um `TextInput` de busca no `ListHeaderComponent` que filtra por nome; quando nada casa, o estado vazio precisa dizer isso, e **não** a mesma frase do primeiro acesso.
5. **Um `BotaoAcao`** reutilizável (o da Prática 3), usado em pelo menos dois lugares, substituindo qualquer `<Button>` ou `<Text onPress>` que tenha sobrado da Aula 3.
...
```tsx
// data/pets.ts — o mock cresce: no mínimo 20 itens, em pelo menos 3 grupos.
...
import { type Pet, type StatusPasseio } from '@/types';
...
export function ItemPet({ pet, onAlternar, onRemover }: ItemPetProps) {
  return (
    <Pressable
      ...
      style={/* TODO 9 */}
    >
      {/* TODO 10: reaproveite o CardPet da Aula 3 aqui dentro.
                   Se o CardPet não couber, explique no README por que você mudou. */}
    </Pressable>
  );
}
```
...
export default function TelaPets() {
  ...
  // TODO 13: alternar status do passeio — array novo, objeto novo. Nada de mutação.
...
## Prática Aplicada 2 — Auditoria de lista em código alheio
...
## Prática Aplicada 3 — Refazer a lista da Aula 3 *(opcional, bônus)*

**Nível:** ⭐ · **Tempo:** ~30 min

Pegue a tela que você entregou na Prática Aplicada da **Aula 3** e converta o que era `.map()` dentro de `ScrollView` para `FlatList`.
...
<details>
<summary><b>Gabarito — Prática 1</b></summary>
...
<details>
<summary><b>Gabarito — Prática 2</b></summary>
...
| 2 | `keyExtractor={(item, index) => String(index)}` | Chave por posição | Ao registrar um passeio e a lista reordenar/filtrar, o estado "pula" para o pet errado | Remover a prop — o default já usa `item.id` |
| 3 | `onPressIn={() => registrarPasseio(item.id)}` | Ação no callback de "encostou" | O passeio é registrado antes de soltar; arrastar para fora não cancela | `onPress` |
...
| 5 | `alvo.statusPasseio = 'concluido'; setPets(pets)` | Mutação: o array continua sendo o mesmo | Nada muda na tela ao tocar | `setPets((a) => a.map((p) => p.id === id ? { ...p, statusPasseio: 'concluido' } : p))` |
...
| 7 | `{item.nome}` solto dentro do `Pressable` | Texto fora de `<Text>` (Aula 3) | Erro em runtime: *Text strings must be rendered within a `<Text>` component* | `<Text>{item.nome}</Text>` |
...
<details>
<summary><b>Gabarito — Prática 3</b></summary>
...
<details>
<summary><b>Gabarito — Prática 4</b></summary>
...
<details>
<summary><b>Gabarito — Prática 5</b></summary>
...
function ListaVazia() {                                           // TODO 2
  return (
    <View style={styles.vazio}>
      <Text style={styles.vazioTitulo}>Nenhum pet por aqui</Text>
      <Text style={styles.legenda}>Toque em “Novo pet” para começar.</Text>
    </View>
  );
}

const Cabecalho = <Text style={styles.cabecalho}>Meus pets</Text>;         // TODO 3

function ItemPet({ pet, onAlternar }: ItemProps) {
  const passeouHoje = pet.statusPasseio === 'concluido';
  return (
    <Pressable
      onPress={() => onAlternar(pet.id)}                          // TODO 4
  ...
  return (
    <View style={styles.tela}>
      <FlatList
        ...
        //         Pet tem id. Escrever keyExtractor aqui só repetiria o default.
...
**Sobre o TODO 3:** `Cabecalho` é uma **constante com o elemento pronto**, definida fora do componente. Se fosse `ListHeaderComponent={() => <Text>...</Text>}`, uma arrow anônima nova nasceria a cada render, o React trataria como um tipo de componente diferente e remontaria o cabeçalho. Com um `<Text>` simples ninguém percebe; com um `TextInput` de busca lá dentro (Prática Aplicada 1), o campo perde o foco a cada tecla.

**Sobre o TODO 7:** o `map` cria array novo e o spread cria objeto novo. Se você tivesse feito `pets.find(...).statusPasseio = 'concluido'`, o array continuaria idêntico, a `FlatList` — que é `PureComponent` — não veria diferença nenhuma, e a tela não mudaria.
...
<details>
<summary><b>Gabarito — Prática 6</b></summary>

```tsx
function agruparPorStatus(pets: Pet[]): Secao[] {
  const rotulos: Record<StatusPasseio, string> = {                // TODO 1
    pendente:  'Passeio pendente',
    concluido: 'Passeio concluído',
    cancelado: 'Passeio cancelado',
  };
  const ordem: StatusPasseio[] = ['pendente', 'concluido', 'cancelado'];   // TODO 2

  return ordem
    .map((status) => ({                                           // TODO 3
      title: rotulos[status],        // campo SEU — poderia se chamar qualquer coisa
      data: pets.filter((p) => p.statusPasseio === status),   // campo obrigatório da API
    }))
...
export default function TelaSecoes({ pets }: { pets: Pet[] }) {
  return (
    <View style={styles.tela}>
      <SectionList
        sections={agruparPorStatus(pets)}                         // TODO 5
        renderItem={({ item }) => <ItemPet pet={item} onAlternar={alternar} />}   // TODO 6
        renderSectionHeader={({ section }) => (                   // TODO 7
          <Text style={styles.cabecalhoSecao}>{section.title}</Text>
        )}
        renderSectionFooter={({ section }) => (                   // TODO 8
          <Text style={styles.rodapeSecao}>{section.data.length} pet(s)</Text>
        )}
...
**Sobre o TODO 2 — a ordem é uma decisão:** `['pendente', 'concluido', 'cancelado']` coloca o que exige ação do usuário em cima. Ordem alfabética seria "cancelado, concluído, pendente", que enterra o grupo mais importante no fim. O componente não tem opinião sobre isso; você tem.
...
**Pergunta final:** você **não** precisou mudar nada no `ItemPet`. Isso mostra que a diferença entre `FlatList` e `SectionList` está no **formato do dado** e no **cabeçalho por grupo** — não em como o item é renderizado. Os dois são `VirtualizedList` por baixo, e quase todas as props são as mesmas.

</details>

<details>
<summary><b>Gabarito — Prática 7</b></summary>
...
Mesmo com o `Set` novo e o componente re-renderizando (o contador no topo passa a funcionar), **os itens continuam iguais**: a prop `data` da `FlatList` é o mesmo array `pets` de sempre, e o dado que mudou (`selecionados`) não está dentro dele. Como as props da lista continuam shallow-equal, ela não re-renderiza as linhas.

**3. As duas correções possíveis:**

```tsx
// Correção A — extraData: avisa a lista de que algo de FORA do data mudou
<FlatList data={pets} extraData={selecionados} renderItem={...} />

// Correção B — mover a seleção para dentro do dado
const [pets, setPets] = useState(MOCK.map((p) => ({ ...p, selecionado: false })));
// e alternar com map + spread, como na Prática 5
```

**Qual escolher:** aqui, a **A**. `extraData` existe exatamente para este caso — estado de interface que é legitimamente separado do dado de domínio. "Está selecionado nesta tela" não é uma propriedade do pet; é uma propriedade da tela. A correção B funciona, mas polui o modelo de dados com um campo que só faz sentido enquanto essa tela específica está aberta.

Compare com a Prática 5: lá, "o passeio foi concluído" **é** propriedade do pet, e por isso a resposta certa era a imutabilidade, não `extraData`. **A pergunta que decide é sempre a mesma: isso é dado do domínio ou estado de tela?**

**4. O que falta:** `ListEmptyComponent` (regra 8). Também falta `flex: 1` garantido no contêiner e o `renderItem` está inline — este último é aceitável aqui pelo mesmo motivo da Prática 5.
...
Se precisar cruzar com o material do outro projeto (`exercises.md`), esta é a tradução. **A lógica das práticas não muda** — só os nomes.

| Habito (`exercises.md`) | Pet (este arquivo) |
|---|---|
| `Habito` | `Pet` |
| `ItemHabito` | `ItemPet` |
| `CardHabito` | `CardPet` |
...
| "Novo hábito" | "Novo pet" |
| "Marcar concluído" | "Registrar passeio" |
| `filtrarPorTitulo` | `filtrarPorNome` |
...
| Parte | Onde | Tempo |
|---|---|---|
| Práticas 1, 2 e 4 | Sala, sem computador | ~13 min |
| Práticas 3, 5 e 6 | Sala, no computador | ~25 min |
| Prática 7 | Sobra / casa | ~5 min |
| Prática Aplicada 1 | Casa | 3–4 h |
| Prática Aplicada 2 | Casa | ~1 h |
| Prática Aplicada 3 *(bônus)* | Casa | ~30 min |
